...
import pandas as pd
import os
def collect_stats_and_save_to_csv(player_stats):
    # Create a list to hold all the stats for each frame
    collected_stats = []

    for index, row in player_stats.iterrows():
        stats = {
            "Frame Index": index,
            "Player 1 Shot Speed (km/h)": row['player_1_last_shot_speed'],
            "Player 2 Shot Speed (km/h)": row['player_2_last_shot_speed'],
            "Player 1 Speed (km/h)": row['player_1_last_player_speed'],
            "Player 2 Speed (km/h)": row['player_2_last_player_speed'],
            "Player 1 Average Shot Speed (km/h)": row['player_1_average_shot_speed'],
            "Player 2 Average Shot Speed (km/h)": row['player_2_average_shot_speed'],
            "Player 1 Average Speed (km/h)": row['player_1_average_player_speed'],
            "Player 2 Average Speed (km/h)": row['player_2_average_player_speed']
        }
    ...
    stats_df.to_csv(os.getenv('video_path')+".csv", index=False)
...
def draw_player_stats(output_video_frames,player_stats):
    ...
    stats_df = collect_stats_and_save_to_csv(player_stats)
    print("Statistics have been saved to CSV.")
